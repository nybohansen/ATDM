...
[
	...
	if test "x$want_boost" = "xyes"; then
		...
		# if .a file is available, link explicitly to that file
		# done to ensure static linking on "OS X"
		AC_MSG_NOTICE("$BOOSTLIBDIR/lib$ax_lib.a")
		if test -f "$BOOSTLIBDIR/lib$ax_lib.a"; then
		   BOOST_SERIALIZATION_LIB="$BOOSTLIBDIR/lib$ax_lib.a"
		   AC_SUBST(BOOST_SERIALIZATION_LIB)
        	   AC_MSG_NOTICE("Replaced -l$ax_lib with explicit linking: $BOOSTLIBDIR/lib$ax_lib.a")
		fi
