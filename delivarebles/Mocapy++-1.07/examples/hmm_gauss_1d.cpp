...
int main(void) {
	...
	means.set_shape(2,1); means.set_values( vec(0.0,6.0) );

	MDArray<double> covs;
	covs.set_shape(2,1,1); covs.set_values( vec(1.0, 2.0) );
	...
	Node* to0 = NodeFactory::new_gaussian_node(O_SIZE, "to0", false, false, means, covs);
	...
	Node* mo0 = NodeFactory::new_gaussian_node(O_SIZE, "mo0", true);
